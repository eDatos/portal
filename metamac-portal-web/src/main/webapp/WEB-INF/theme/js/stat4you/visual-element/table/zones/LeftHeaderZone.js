(function () {
    ...
    var Size = STAT4YOU.Table.Size,
        ...
        Utils = STAT4YOU.Table.Utils;

    STAT4YOU.namespace("STAT4YOU.Table.LeftHeaderZone");

    STAT4YOU.Table.LeftHeaderZone = function (options) {
        STAT4YOU.Table.Zone.prototype.initialize.apply(this, arguments);
        this.initialize(options);
    };

    _.extend(STAT4YOU.Table.LeftHeaderZone.prototype, STAT4YOU.Table.Zone.prototype);

    STAT4YOU.Table.LeftHeaderZone.prototype.initialize = function (options) {
        if (!options) {
            options = {};
        }
    ...
    STAT4YOU.Table.LeftHeaderZone.prototype.calculateIncrementalSize = function () {
    ...
    STAT4YOU.Table.LeftHeaderZone.prototype.paintInfo = function () {
        ...
        for (var i = 0; i < columnsLen; i++) {
            ...
            for (var j = 0; j < bodyPaintInfo.rows.length; j++) {
                ...
                if (result[i].length === 0 || _.last(result[i]).index != index) {
                    ...
                    var content = columnsValues[i][indexInValue % columnsValuesLength[i]];
    ...
    STAT4YOU.Table.LeftHeaderZone.prototype.cellAtPoint = function (absolutePoint) {
    ...
    STAT4YOU.Table.LeftHeaderZone.prototype.titleAtPoint = function (absolutePoint) {

        var headerCellAtPoint = this.cellAtPoint(absolutePoint);
        if(headerCellAtPoint) {
            return headerCellAtPoint.content
        }
    };

    STAT4YOU.Table.LeftHeaderZone.prototype.rowsAtPoint = function (absolutePoint) {
        var headerCellAtPoint = this.cellAtPoint(absolutePoint);
        if(headerCellAtPoint) {
    ...
    STAT4YOU.Table.LeftHeaderZone.prototype.repaint = function () {
        this.clear();
    ...
    STAT4YOU.Table.LeftHeaderZone.prototype.paintShadow = function () {
    ...
    STAT4YOU.Table.LeftHeaderZone.prototype.paintBorders = function () {
        var columnsLen = this.dataSource.leftHeaderColumns();
        this.ctx.save();
        this.ctx.beginPath();
    ...
    STAT4YOU.Table.LeftHeaderZone.prototype.paintCells = function (paintInfo) {
        this.ctx.save();
