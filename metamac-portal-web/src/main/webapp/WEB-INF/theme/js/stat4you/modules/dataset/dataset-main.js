(function () {
    ...
    STAT4YOU.modules.dataset.Main.prototype = {
...
//            };
            ...
            self.filterOptionsByType = _.chain(veElements)
                .map(function (veElement) {
                    var veElementFilterOptions = new STAT4YOU.widget.FilterOptions({metadata : self.metadata});
            ...
            if (options.showPage) {
                self.pageView = new STAT4YOU.modules.dataset.PageView({
                    el : options.el,
                    metadata : self.metadata,
                    attributes : self.attributes
                });
            } else {
                self.pageView = new STAT4YOU.modules.dataset.DatasetWidgetPageView({
                    el : options.el,
                    metadata : self.metadata,
                    attributes : self.attributes,
            ...
            self.exportView = new STAT4YOU.modules.dataset.ExportView({el : exportViewEl, model : self.optionsModel});
            self.exportView.render();

            self.permalinkBuilder = new STAT4YOU.modules.dataset.PermalinkBuilder({
                filterOptions : self.filterOptions,
                optionsModel : self.optionsModel
            });

            var actions$el = self.pageView.$el.find(".dataset-header-actions");
            self.datasetActionsView = new STAT4YOU.modules.dataset.DatasetActionsView({
                el : actions$el,
                filterOptions : self.filterOptions,
            ...
            self.exporter = new STAT4YOU.svg.Exporter();
            ...
            // FullScreen
            self.fullScreen = new STAT4YOU.FullScreen({container : $sidebarContainer});
            ...
            self.fullScreen.on('didEnterFullScreen', function () {
                ...
                STAT4YOU.track({event : 'dataset.fullScreen.open', datasetUri : self.metadata.getUri() });
            });
            ...
            self.fullScreen.on('didExitFullScreen', function () {
                ...
                STAT4YOU.track({event : 'dataset.fullScreen.close', datasetUri : self.metadata.getUri() });
            });
            ...
            self.optionsModel.on('change:type', function (model, type) {
                self.datasetActionsView.render();
                STAT4YOU.track({event : 'dataset.view', veType : type, datasetUri : self.metadata.getUri() });
            });

            self.router.on('changeChart', function (options) {
                if (options.selection) {
                    var permalink = new STAT4YOU.modules.dataset.DatasetPermalink({identifier : options.selection});
            ...
            STAT4YOU.modules.signin.SigninView.prototype.redirectUrl = function () {
