STAT4YOU.namespace("STAT4YOU.widget.filter");

STAT4YOU.widget.filter.FilterView = Backbone.View.extend({
    _templateExternalContainer : STAT4YOU.templateManager.get('widget/filter/filter-external-container'),
    _templateTableContainers : STAT4YOU.templateManager.get('widget/filter/filter-table-containers'),
    _templateColumnContainers : STAT4YOU.templateManager.get('widget/filter/filter-column-containers'),
    _templateLineContainers : STAT4YOU.templateManager.get('widget/filter/filter-line-containers'),
    _templatePieContainers : STAT4YOU.templateManager.get('widget/filter/filter-pie-containers'),
    ...
    _renderTableDimensions : function() {
        ...
        _.each(leftZoneContext.dims, function(dim){this._createDimension(dim, '#div-left-dimensions');}, this);
        _.each(topZoneContext.dims, function(dim){this._createDimension(dim, '#div-top-dimensions');}, this);
        _.each(fixedZonecontext.dims, function(dim){this._createDimension(dim, '#div-fixed-dimensions');}, this);
    ...
    _handleDrop : function(e) {
        ...
        var oldParent = $(oldDimension).parents('.div-dimension-long, .div-dimension-container').get(0);
    ...
    _createDimension : function(dim, zoneId) {
        ...
        var tempDimensionView = new STAT4YOU.widget.filter.FilterDimensionView({model: this.model, el : tempEl, zone: $zone.data("zone") });
    ...
    save : function() {
        ...
        this.trigger('saveFilter');// TODO: Maybe the options-view should have a listener in its "events"
    },
    ...
    close : function() {
        ...
        this._destroyTmpFilterOptions();
        this.$filterContainer.remove();
        this.trigger('closeFilter'); // TODO: Maybe the options-view should have a listener in its "events"
    }
