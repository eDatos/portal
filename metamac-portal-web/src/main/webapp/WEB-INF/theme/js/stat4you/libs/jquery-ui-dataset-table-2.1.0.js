...
(function($) {
    ...
    $.widget("ui.datasetTable", {
        ...
        setData : function() {
            ...
            /* SCROLLBARS */
            if (self.$dataCells.width() > self.$dataContainer.width())
                self.$scrollBottom.show();
            if (self.$dataCells.height() > self.$dataContainer.height())
...
//            var $toDelTopAndLeft = $('.dataset-columns-cells');
//            for (var i=0; i<$toDelTopAndLeft.length; i++) {
//                var tempDiv = $toDelTopAndLeft.get(i);
//                while(tempDiv.children.length > 0) {
//                    tempDiv.removeChild(tempDiv.firstChild);
//                }
//            }
//            var $toDeldata = $('.dataset-data-cells');
//            var tempDiv = $toDeldata.get(0);
//            while(tempDiv.children.length > 0) {
//                tempDiv.removeChild(tempDiv.firstChild);
//            }
...
//            self.setData();
        ...
        _createColumnTop : function() {
            ...
            for (var i=0; i<numDims; i++) {
                ...
                var $currentTopDimContainer = $("<div id='topContainerDim"+i+"' class='columnsTopDimContainer' style='height: "+height+"px' >").appendTo(self.$columnsTopCells);
                var j=0;
...
//                console.log("AFTER..... oldLeft %s", oldLeft);
        ...
        _createDataGrid : function() {
            ...
            var gridElementWidth = self._firstPossibleSize(minWidth, options.columnTop.width, true);

            self.numGridsElementsPerGridWidth = Math.ceil(containerWidth / gridElementWidth);
            ...
            var gridElementHeight = self._firstPossibleSize(minHeight, options.columnTop.height, true);

            self.numGridsElementsPerGridHeight = Math.ceil(containerHeight / gridElementHeight);
        ...
        _populateGridElement : function(gridElement) {
            var self = this,
                ...
                tempCellIdStr = self.firstElementChild(gridElement).id,
            ...
            while (v <= self.numCellsPerGridElemHeight) {
                ...
                while (h <= self.numCellsPerGridElemWidth) {
                    ...
                    for (var i=options.columnTop.dimensions.length-1; i>=0; i--) {
                        ...
                        if (options.columnTop.dimensions[i].representations[categoryIndex])
                    ...
                    for (var i=options.columnLeft.dimensions.length-1; i>=0; i--) {
                        ...
                        /* To avoid null values */
                        if (options.columnLeft.dimensions[i].representations[categoryIndex])
        ...
        _gridMoveStepRepopulate : function() {
            ...
            for (var i=0; i<rows.length; i++) {
                ...
                for (var j=0; j<gridElements.length; j++) {
                    // Populating the DIV
                    self.numCurrentGridElemPerWidth = parseInt((self.gridLeftOffset + self.$gridElement.width() * j) / self.$gridElement.width());
        ...
        _gridMoveLeftRight : function(leftDataNew) {
            ...
            var step = 0;
            // RIGHT
            if ((leftDiffContainerGrid > movingRightThreshold) && self._isMovingRight(self.gridLeftOffset, leftDataNew)) {
                ...
                for (var i=0; i<rows.length; i++) {
                    ...
                    row.appendChild(self.firstElementChild(row));
                    // Populate
                    self.numCurrentGridElemPerWidth = parseInt((self.gridLeftOffset + self.$gridElement.width() * 2) / self.$gridElement.width());
                    self.numCurrentCellPerGridWidthInit = self.numCurrentGridElemPerWidth * self.numCellsPerGridElemWidth;
                    self.numCurrentGridElemPerHeight = parseInt((self.gridTopOffset + self.$gridElement.height() * i) / self.$gridElement.height());
                    self.numCurrentCellPerGridHeightInit = self.numCurrentGridElemPerHeight * self.numCellsPerGridElemHeight;
                    self._populateGridElement(self.lastElementChild(row));
                }
            }
            // LEFT
            else if ((leftDiffContainerGrid <= movingLeftThreshold) && self._isMovingLeft(self.gridLeftOffset, leftDataNew)) {
                step -= self.$gridElement.width();
                self.gridLeftOffset += step;
                self.$grid.css("left", self.gridLeftOffset);
                var rows = $(".gridRow", self.$grid);
                ...
                for (var i=0; i<rows.length; i++) {
                    ...
                    row.insertBefore(self.lastElementChild(row), self.firstElementChild(row));
                    // Populate
                    self.numCurrentGridElemPerWidth = parseInt((self.gridLeftOffset + 0) / self.$gridElement.width());
                    self.numCurrentCellPerGridWidthInit = self.numCurrentGridElemPerWidth * self.numCellsPerGridElemWidth;
                    self.numCurrentGridElemPerHeight = parseInt((self.gridTopOffset + self.$gridElement.height() * i) / self.$gridElement.height());
                    self.numCurrentCellPerGridHeightInit = self.numCurrentGridElemPerHeight * self.numCellsPerGridElemHeight;
                    self._populateGridElement(self.firstElementChild(row));
                }
            }
        ...
        _gridMoveTopBottom : function(topDataNew) {
            ...
            var step = 0;
            // BOTTOM
            if ((topDiffContainerGrid > movingBottomThreshold) && self._isMovingBottom(self.gridTopOffset, topDataNew)) {
                step += self.$gridElement.height();
                self.gridTopOffset += step;
                self.$grid.css("top", self.gridTopOffset);
                var grid = self.$grid.get(0);
                // Now the row is the last one
                grid.appendChild(self.firstElementChild(grid));
                // Populate
                self.numCurrentGridElemPerHeight = parseInt((self.gridTopOffset + self.$gridElement.height() * 2) / self.$gridElement.height());
                self.numCurrentCellPerGridHeightInit = self.numCurrentGridElemPerHeight * self.numCellsPerGridElemHeight;
                for (var i=0; i< self.lastElementChild(grid).children.length; i++) {
                    self.numCurrentGridElemPerWidth = parseInt((self.gridLeftOffset + self.$gridElement.width() * i) / self.$gridElement.width());
                    self.numCurrentCellPerGridWidthInit = self.numCurrentGridElemPerWidth * self.numCellsPerGridElemWidth;
                    self._populateGridElement(self.lastElementChild(grid).children[i]);
                }
            }
            // TOP
            else if ((topDiffContainerGrid <= movingTopThreshold) && self._isMovingTop(self.gridTopOffset, topDataNew)) {
                step -= self.$gridElement.height();
                self.gridTopOffset += step;
                self.$grid.css("top", self.gridTopOffset);
                var grid = self.$grid.get(0);
                grid.insertBefore(self.lastElementChild(grid), self.firstElementChild(grid));
                // Populate
                self.numCurrentGridElemPerHeight = parseInt((self.gridTopOffset + 0) / self.$gridElement.height());
                self.numCurrentCellPerGridHeightInit = self.numCurrentGridElemPerHeight * self.numCellsPerGridElemHeight;
                for (var i=0; i< self.firstElementChild(grid).children.length; i++) {
                    self.numCurrentGridElemPerWidth = parseInt((self.gridLeftOffset + self.$gridElement.width() * i) / self.$gridElement.width());
                    self.numCurrentCellPerGridWidthInit = self.numCurrentGridElemPerWidth * self.numCellsPerGridElemWidth;
                    self._populateGridElement(self.firstElementChild(grid).children[i]);
                }
            }
        ...
        _mouseUp : function(e) {
            ...
            if (moveState.moveTop && moveState.moveLeft) {
                self.$dataCells.css("cursor", "pointer");
            } else if (moveState.moveTop) {
                self.$columnsLeftCells.css("cursor", "pointer");
            } else if (moveState.moveLeft) {
                self.$columnsTopCells.css("cursor", "pointer");
            }
         ...
         * GRID
         * --------------------------------------------- */
        _isMovingRight : function(previousLeft, newLeft) {
            return (previousLeft < newLeft);
        },
        
        _isMovingLeft : function(previousLeft, newLeft) {
            return (previousLeft > newLeft);
        },
        
        _isMovingBottom : function(previousTop, newTop) {
            return (previousTop < newTop);
        },
        
        _isMovingTop : function(previousTop, newTop) {
            return (previousTop > newTop);
        },
         ...
         *******************************************************************************/
        _firstPossibleSize : function(initialDivisibleNumber, restriction, forward) {
            var toret = Math.round(initialDivisibleNumber);
            var temp = -1;
            var found = false;
            while (!found) {
                temp = parseInt(toret % restriction);
                if (temp == 0) {
                    found = true;
                }
                else
        ...
        _firstPossibleSizeTwoRestrictions : function(initialDivisibleNumber, numberToDivide, restriction, forward) {
        ...
        lastElementChild : function (el) {
            return (el.lastElementChild || el.children[el.children.length - 1] || {} );
        },        
        ...
        _initializeStructure : function() {
            ...
            self.$title = $("<h3>"+self.options.title.text+"</h3>").appendTo(self.$root);
            
            self.$dataset = $("<div />").appendTo(self.$root);
        ...
        _initializeSizes : function() {
            ...
            self.columnsLeftHeight = options.height - self.columnsTopHeight;
        ...
        _initializeScrollBars : function() {
            ...
            if (self.dataWidth < self.dataCellsWidth) {
                ...
                //self.$scrollBottomDrag.bind('mousedown', function (e) {
                //    e.stopPropagation();
                //});
                //self.$scrollBottomDrag.bind('mouseup', function (e) {
                //    e.stopPropagation();
                //});
            }
            else {
