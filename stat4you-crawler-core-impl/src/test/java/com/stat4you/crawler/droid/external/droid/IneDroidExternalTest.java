...
public class IneDroidExternalTest {
    ...
    public void setUp() throws Exception {
        this.crawlerInfo = (CrawlerInfo) ApplicationContextProvider.getApplicationContext().getBean("ineCrawlerInfo");
    ...
    public void testForceUpdateCrawlingDroid() throws Exception {
        ForceUpdateInfo forceUpdateInfoMode = ForceUpdateInfoBuilder.forceUpdateInfo().withCategory("05").withPeriod("2005").withRegion("00")
                .withStartUrl("http://www.ine.es/jaxiBD/menu.do?L=0&divi=IPR&his=2&type=db").build();
    ...
    private void crawlingDroid(List<String> locations) throws DroidsException, InterruptedException, ApplicationException {
        ((PxHandler) ApplicationContextProvider.getApplicationContext().getBean("inePxHandler")).setImportationService(mock(ImportationService.class));
    ...
    private Droid<Link> createSimpleReportCrawlingDroid(List<String> locations) {
        Droid<Link> droid = (Droid<Link>) ApplicationContextProvider.getApplicationContext().getBean("ineCrawler");
