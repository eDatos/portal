...
public class EustatDroidExternalTest {
    ...
    public void setUp() throws Exception {
        this.crawlerInfo = (CrawlerInfo) ApplicationContextProvider.getApplicationContext().getBean("eustatCrawlerInfo");
    ...
    public void testForceUpdateCrawlingDroid() throws Exception {
        ForceUpdateInfo forceUpdateInfoMode = ForceUpdateInfoBuilder.forceUpdateInfo().withCategory("Territorio y medio ambiente")
                .withStartUrl("http://www.eustat.es/bancopx/spanish/id_2385/indiceRR.html#axzz1x66J8oVS").build();
    ...
    private void crawlingDroid(List<String> locations) throws DroidsException, InterruptedException, ApplicationException {
        ((PxHandler) ApplicationContextProvider.getApplicationContext().getBean("eustatPxHandler")).setImportationService(mock(ImportationService.class));
    ...
    private Droid<Link> createSimpleReportCrawlingDroid(List<String> locations) {
        Droid<Link> droid = (Droid<Link>) ApplicationContextProvider.getApplicationContext().getBean("eustatCrawler");
