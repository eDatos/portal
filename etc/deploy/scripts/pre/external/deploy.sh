...
ssh -o "StrictHostKeyChecking=no" deploy@mgcartera.gobiernodecanarias.net <<EOF
    ...
    sudo unzip $DEPLOY_TARGET_PATH/statistical-visualizer.war -d $DEPLOY_TARGET_PATH/istac#statistical-visualizer
    sudo rm -rf $DEPLOY_TARGET_PATH/statistical-visualizer.war
    
    # Restore Configuration
    sudo cp -rf $ENV_CONF/web/* $DEPLOY_TARGET_PATH/istac#statistical-visualizer
    ...
    sudo unzip $DEPLOY_TARGET_PATH/statistical-visualizer-api.war -d $DEPLOY_TARGET_PATH/api#estadisticas#statistical-visualizer
    sudo rm -rf $DEPLOY_TARGET_PATH/statistical-visualizer-api.war

    # Restore Configuration
    sudo cp -rf $ENV_CONF/api/* $DEPLOY_TARGET_PATH/api#estadisticas#statistical-visualizer
