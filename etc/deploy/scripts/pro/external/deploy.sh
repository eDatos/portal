...
ssh -o "StrictHostKeyChecking=no" deploy@mgcrinaco.gobiernodecanarias.net <<EOF
    ...
    sudo unzip $DEPLOY_TARGET_PATH/statistical-visualizer-api.war -d $DEPLOY_TARGET_PATH/istac#api#statistical-visualizer
    sudo rm -rf $DEPLOY_TARGET_PATH/statistical-visualizer-api.war

    # Restore Configuration
    sudo cp -rf $ENV_CONF/api/* $DEPLOY_TARGET_PATH/istac#api#statistical-visualizer
